...
class DungeonMazeEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, grid_size=16, use_shaping=True):
 
        self.grid_size = grid_size
        self.window_size = 512
        self.max_steps = 200  # or any reasonable limit to prevent infinite episodes
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "left_blocked": spaces.Discrete(2),
                "right_blocked": spaces.Discrete(2),
            }
        )
    ...
    def get_observations(self):
        """
        Returns a dictionary containing the robot's position, direction, camera view,
        target position, and left/right wall sensors.
        """
        return {
            ...
            "left_blocked": int(self._is_cell_blocked(self.get_robot_left_pos())),
            "right_blocked": int(self._is_cell_blocked(self.get_robot_right_pos())),
        }
    ...
    def get_robot_left_pos(self):
        """Get the position of the cell immediately to the robot's left."""
        direction_vectors = [
            np.array((0, -1)),   # North
            np.array((1,  0)),   # East
            np.array((0,  1)),   # South
        ...
        return self.robot_position + direction_vectors[left_dir]

    def get_robot_right_pos(self):
        """Get the position of the cell immediately to the robot's right."""
        direction_vectors = [
            np.array((0, -1)),   # North
            np.array((1,  0)),   # East
            np.array((0,  1)),   # South
        ...
        return self.robot_position + direction_vectors[right_dir]

    def _is_cell_blocked(self, pos):
        """Return True if pos contains a non-overlappable object (wall) or is out of bounds."""
    ...
    def reset(self, seed=None, options=None):
        ...
        """
        self.current_step = 0
        self.last_positions = []  # ✅ reset loop-detection history per episode
    ...
    def step(self, action):
        ...
        # Store position BEFORE action for distance reward
        ...
        if action == Actions.turn_left:
            ...
            # Compute what's ahead ONLY when actually moving
            position_in_front = self.get_robot_front_pos()
            cell_in_front = self.maze.get_cell_item(*position_in_front)
            ...
            if cell_in_front is None or cell_in_front.can_overlap():
                ...
                reward = -0.5  #hit wall penalty

        # ===== DISTANCE REWARD =====
        # Only applied when use_shaping=True. Disabled for DQN training because
        # the Euclidean-proximity signal misleads the neural network in mazes:
        # it learns to value closeness in straight-line distance, not path quality.
        if self.use_shaping:
            old_distance = np.linalg.norm(old_position - self.target_position)
            new_distance = np.linalg.norm(self.robot_position - self.target_position)
